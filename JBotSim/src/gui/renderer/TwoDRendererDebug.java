					...
					if (s.getClass().getSimpleName().equals(coneClass) || s.getId() == coneSensorId) {
						if (s != null && s instanceof ConeTypeSensor) {
							...
							for (int i = 0; i < preySensor.getAngles().length; i++) {
								...
								int a1 = (int) (FastMath.round(FastMath.toDegrees(preySensor.getSensorsOrientations()[i]
										+ robot.getOrientation() - openingAngle / 2)));
								...
								if (cutOff > 0) {
									...
									graphics2D.setColor(new Color(192, 192, 192, 127));
									graphics2D.fillOval(x3, y3, (int) FastMath.round(cutOff * 2 * scale),
											(int) (FastMath.round(cutOff * 2 * scale)));
								}
