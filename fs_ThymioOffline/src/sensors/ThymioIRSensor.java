...
public class ThymioIRSensor extends ConeTypeSensor {
	...
	private final double NAIVE_CUTOFF = 4500;
	...
	public ThymioIRSensor(Simulator simulator, int id, Robot robot, Arguments args) {
		...
		naiveSensor = args.getArgumentAsInt("naivesensor") == 1;
	...
	private double distanceToSensor(double distance, int sensorNumber) {
		...
		if(naiveSensor){
			...
			double reading = (getRange() - distance)/getRange();
			if(this.noiseEnabled){
				reading += random.nextGaussian()*NOISESTDEV * reading;
			}
			reading = reading * NAIVE_CUTOFF;
		...
		double[] sensorReferences = sensorReferenceValues[sensorNumber];
		double[] stdDeviations = sensorReferenceStdDeviation[sensorNumber];
		...
		if (distance < distances[distances.length - 1]) {
			...
			for (i = 0; i < distances.length; i++) {
				if (distance <= distances[i]) {
					...
					if (noiseEnabled)
						currentSensorValue += random.nextGaussian() * stdDeviations[i];
